...
Документ — исторический журнал рефакторинга сетевого слоя: что было сделано и почему, этап
за этапом. Он не описывает текущую архитектуру как таковую — снимок текущего состояния см.
в [`docs/architecture.md`](architecture.md) — и не планирует будущие изменения: планирование
ведётся через GitHub Issues. Ориентирован на то, чтобы к истории решений можно было
вернуться без дополнительного контекста — только по коду и этому файлу.
